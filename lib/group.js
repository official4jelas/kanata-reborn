...
export async function groupParticipants(ev, sock) {
    ...
    for (const participant of participants) {
        ...
        switch (action) {
            ...
            case 'remove':
                if (settings.goodbye) {
                    const goodbyeMsg = settings.goodbye_message
                        .replace('@user', `@${userId}`)
                        .replace('@group', groupName);
                    await sendMessage(participant, { 
                        text: goodbyeMsg,
            ...
            case 'promote':
                await sendMessage(participant, { 
                    text: `🎉 Selamat @${userId}! Kamu sekarang menjadi Admin.`, 
                    mentions: [participant] 
                });
                break;
            case 'demote':
                await sendMessage(participant, { 
                    text: `⚠️ @${userId} telah diturunkan dari admin.`, 
                    mentions: [participant] 
                });
                break;
        }
