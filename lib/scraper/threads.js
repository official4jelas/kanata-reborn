...
const threads = async (link) => {
    const idMatch = link.match(/\/post\/([^?\/]+)/);
    if (!idMatch || !idMatch[1]) {
        throw new Error("Invalid URL: Unable to extract post ID.");
    }
    ...
    const title = $('body section.download_result_section div.container div.download__item__caption__text').text();
    const author = $('body section.download_result_section div.download__item__profile_pic div span').text();
    const downloadUrl = $('body section.download_result_section div.download_item_info table tbody tr td a.download__item__info__actions__button').attr('href');
